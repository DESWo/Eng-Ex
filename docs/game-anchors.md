# Familiar-game anchors

Every game is based on a popular game archetype students already know, so the
first ten seconds feel familiar and the engineering sneaks in behind it.
Trademarked names stay out of the app copy; the in-game hook uses the
archetype ("a certain bird game", "the arcade claw machine").

Three depths of anchoring:

- **deep**: the interaction itself is the familiar game (drag mechanics, art,
  win feel rebuilt around it).
- **native**: the game already IS the archetype; the level 1 copy names it.
- **hook**: the mechanic matches; the level 1 copy frames it in the familiar
  game's terms.

| Game | Anchor | Depth |
| --- | --- | --- |
| Catapult Lab | Angry Birds (pull-back slingshot, release to fire) | deep |
| Claw Machine (robot-arm) | Arcade claw machine (prizes, cabinet, claw) | deep |
| Shield Stack | Plants vs. Zombies lane defense (rays are the horde, wall is the plants, red ones reach the house) | deep |
| Bridge Builder | Poly Bridge / Bridge Constructor | native |
| Orbit Insertion | Kerbal Space Program's famous burn-forward lesson | native |
| Mission Budget | KSP rocket-builder loadout menu | native |
| Smooth Ride | Hill Climb Racing suspension upgrade screen | hook |
| Balance Act | Playground seesaw | hook |
| Green Wave | Traffic-control sims | (none yet) |
| Shake Proof | Jenga / wobbly block-tower party game | hook |
| Power Up | Mini Metro subway-map routing | hook |
| Circuit Lab | Minecraft-redstone-style wiring sandbox | hook |
| Don't Trip | Tetris-style block fitting (appliances into breaker capacity) | hook |
| Reactor Control | Virtual pet / keep-it-alive sim | hook |
| Decay Heat | (none yet; candidate: idle game in reverse) | - |
| Trim for Flight | Flight sims | hook |
| Re-entry | (its own stone-skipping metaphor already lands) | - |
| Assembly Line | Overcooked kitchen-rush flow | hook |
| Quality Gate | Papers, Please inspection desk (place the desk, watch units get stamped and binned) | deep |
| Warehouse Layout | Unpacking / tidying games, scored in footsteps | hook |
| Line Follower | Kart-racer time-trial tuning | hook |
| Safe Grip | Operation steady-hand buzzer game | hook |
| Clean Stream | (none yet; candidate: pipe-ordering puzzle) | - |
| Solar Array | Stardew-style farm-grid planning | hook |
| Stormwater | City-builder zoning | hook |
| Sky High | (candidate: same block-tower anchor as Shake Proof) | - |
| Beam Section | (none yet) | - |
| Foundation | (none yet) | - |
| Logic Lab | Block-game switch wiring (redstone archetype) | hook |
| Binary Bulbs | Lights Out-style lights puzzle | hook |
| Error Check | (none yet; candidate: sudoku-style parity) | - |
| Backup Plan | Monster-catching team loadout | hook |
| Critical Path | Speedrun route planning | hook |

## Upgrade candidates (hook -> deep)

1. **Don't Trip**: appliances as draggable blocks that visually fill each
   breaker column, Tetris-style.
2. **Shake Proof / Sky High**: exaggerated wobble animation and a
   block-tower art style.
3. **Assembly Line**: stations drawn as kitchen counters with order tickets.
